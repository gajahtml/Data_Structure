...
//--- 스택에서 데이터를 피크(peek, 정상에 있는 데이터를 들여다봄) ---//
	public int peek() throws EmptyIntStackException {
		if (isEmpty()) // 스택이 빔
			new EmptyIntStackException("peek: stack empty!");
		return stk[ptr - 1];
	}
...
//--- 스택 안의 모든 데이터를 바닥 → 정상 순서로 표시 ---//
	public void dump() throws EmptyIntStackException {
		if (isEmpty()) // 스택이 빔
			new EmptyIntStackException("dump: stack empty!");
		System.out.println("스택의 bottom -> top 순으로 출력합니다");
		for(int i = 0; i < ptr; i++) {
...
//--- 스택을 비움 ---//
	public void clear() throws EmptyIntStackException {
		 ...
		 */
		if (isEmpty()) // 스택이 빔
			new EmptyIntStackException("clear: stack empty!");
		ptr = 0;
	}
...
public class Train_4_2_1정수스택_배열_6회차 {
